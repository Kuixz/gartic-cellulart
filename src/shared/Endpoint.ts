...
type BuiltinType = "string" | "number" | "bigint" | "boolean" | "symbol" | "undefined" | "object" | "function" | Function
...
class AsyncEndpoint {
    ...
    constructor(broadcastTag: string, receivingTags: string[] = [], enforceMap?: Record<string, BuiltinType>, enforceWindow: boolean = true) {
        ...
        window.addEventListener('message', (event: CrossCommand) => {
            if (!this.shouldRespondTo(event)) { return }
            const purp = event.data.purpose
            const data = event.data.data
            if (this.enforceMap && purp in this.enforceMap && !enforceType(data, this.enforceMap[purp])) {
                console.warn(`Endpoint received data of wrong type (received ${data} of type ${typeof data} expected type ${this.enforceMap[purp]})`)
                return
            }
            if (!(purp in this.actionMap)) { return }
    ...
    private shouldRespondTo(event: CrossCommand): boolean {
        if (this.enforceWindow && event.source !== window) { return false }
    ...
    post(purp: string, data: any) {
        if (this.enforceMap && purp in this.enforceMap && !enforceType(data, this.enforceMap[purp])) {
            console.warn(`E0X45 HSHFAIL: Endpoint is trying to send data of wrong type (sending ${data} of type ${typeof data} expected type ${this.enforceMap[purp]})`)
            return
        }
        window.postMessage({
