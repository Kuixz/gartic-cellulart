...
class Controller {
  ...
  constructor(modules: ModuleChamber, metamodules: MetaChamber) {
  ...
  unhide() {}
  async createMenu(
    // TODO separate into two functions: initialize ([], [], []) and createMenu([], [])
    ...
    const menuItems = menu.firstElementChild!
...
class Observer {
  ...
  private deduceSettingsFromXHR(data: GarticXHRData) {
    ...
    this.controller.game.user.avatar =
      "https://garticphone.com/images/avatar/" + data.user.avatar + ".svg";

    this.controller.game.players = data.users;
    for (const player of this.controller.game.players) {
      player.avatar =
        "https://garticphone.com/images/avatar/" + player.avatar + ".svg";
    }
  ...
  private deduceSettingsFromSocket(event: Event) {
    ...
    switch (messageType) {
      ...
      case EMessagePurpose.GALLERY_SHOW_TURN: {
        if (messageData.data === undefined && messageData.user !== undefined) {
          return;
        }
      ...
      case EMessagePurpose.CHANGE_SETTINGS_PRESET: {
      ...
      case EMessagePurpose.CHANGE_SETTINGS_DEFAULT: {
        // Return to normal settings when changing tabs
  ...
  private deducePhaseFromDOM(): [Phase, number] | void {
    ...
    if (!newPhaseElement) {
      Console.warn("Cannot find element to read game phase from", "Observer");
      return;
    }
    let newPhase = (newPhaseElement as Element).classList.item(1) as Phase;
    if (!newPhase) {
      Console.warn("Cannot read game phase from selected element", "Observer");
      return;
    }
    ...
    const currentTurn = (() => {
      if (["first", "draw", "write", "memory", "mod"].includes(newPhase)) {
        ...
        if (!step) {
          Console.warn("Could not find turn counter", "Observer");
          return -1;
        }
        if (!step.textContent) {
          Console.warn("Could not read turn counter", "Observer");
          return -1;
        }
        ...
        if (isNaN(turnCount)) {
          Console.warn("Could not parse turn counter", "Observer");
          return -1;
        }
        return turnCount;
      } else {
  ...
  private executePhaseTransition(
    ...
    if (outOfGame(oldPhase) && !outOfGame(newPhase)) {
      this.onroundenter();
    }

    if (!outOfGame(newPhase)) {
      const isReconnection =
        transitionData !== null && "users" in transitionData;
