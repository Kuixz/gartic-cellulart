...
class GarticCompositor {
  ...
  public drawText(
    ...
    while (textLength > max) {
      fontSize -= 5;
      this.context.font =
        fontSize + this.context.font.slice(String(fontSize).length);
    ...
    switch (justify) {
      case "L":
        startingx = x + borderwidth / 2;
        break;
      case "M":
        startingx = x - this.context.measureText(text).width / 2;
        break;
      case "R":
        startingx = x - this.context.measureText(text).width - borderwidth / 2;
        break;
      default:
        throw new Error("Justify orientation not recognised");
    }
  ...
  public drawPFP(pfpURL: string, side: Side): "complete" | HTMLImageElement {
    // todo: An attempt was made to repair tainted canvasses, verify
    const pfp = new Image();
    if (pfpURL.includes("custom-avatars-for-gartic-phone")) {
      Console.warn(
        "SillyV's custom avatars extension doesn't work with Spotlight due to CORS reasons. Please ask SillyV to enable CORS on his S3 buckets.",
        "Spotlight",
      );
      return "complete";
    }
...
export class Spotlight extends CellulartModule {
  // [S1]
  name = "Spotlight";
  ...
  protected onalbumchange(event: AlbumChangeEvent): void {
    ...
    if (event.detail.data === undefined) {
      Console.log(
        "End of timeline reached. Beginning frame grabbing",
        "Spotlight",
      );
  ...
  protected onroundleave() {
    if (this.globalGame.currentPhase != "book") {
      return;
    }
    if (this.isSetTo("off") || length(this.compositedFrameDatas) == 0) {
      return;
    }

    const snapshotFrameData = this.compositedFrameDatas;

    const dlInwindow = new Inwindow("default", {
      close: true,
      visible: true,
      ratio: 1,
    });
  ...
  public menuStep(): Setting {
    if (this.globalGame.currentPhase == "book") {
      // || isNonempty(this.compositedFrameDatas)) {
      Console.warn(
        "Changing Spotlight settings mid-album visualization has disastrous consequences - action blocked",
        "Spotlight",
      );
  ...
  private compositeBackgrounds() {
    ...
    canvas.drawText(
      "HOSTED BY " + this.globalGame.host.toUpperCase(),
      1206,
      82,
      50,
      400,
  ...
  private writeResponseFrames() {
    ...
    if (!timeline) {
      Console.warn("Could not find timeline", "Spotlight");
      return;
    }
    ...
    for (const indices of pairs) {
      if (indices.key < 0) {
        Console.log(
          "Did not participate in this round; no frame will be saved",
          "Spotlight",
        );
        continue;
      }
      if (indices.prev < 0) {
        Console.warn(
          "Could not find fallback; no frame will be saved",
          "Spotlight",
        );
        continue;
      } //prevIndex += modeParameters[game.mode]["fallback"] }
      Console.log(
        `Compositing frame with indices ${indices.key},${indices.prev}`,
        "Spotlight",
      );
  ...
  private determineResponseIndices(slides: Element[]) {
    const indexOfPrevSlide = (key: number): number => {
      ...
      while (i >= 0 && slides[i].querySelector(".empty") != null) {
        i -= this.fallback;
        Console.log(
          `Could not find prompt frame, falling back by ${this.fallback}`,
          "Spotlight",
        );
      }
      return i;
    ...
    for (var keyIndex = 1; keyIndex < slides.length; keyIndex++) {
      if (this.findUsername(slides[keyIndex]) != this.globalGame.user.nick) {
        continue;
      }
  ...
  private compositeResponseFrame(
    slides: Element[],
    keyIndex: number,
    prevIndex: number,
  ) {
    ...
    if (bottleneck == "complete") {
      this.compositedFrameDatas[keyIndex - 1] = canvas.context.getImageData(
        0,
        0,
        1616,
        683,
  ...
  private findUsername(element: Element): string {
    return (
      (element.querySelector(".nick") ?? element.querySelector("span"))
        ?.textContent ?? "missingno"
    );
  }
