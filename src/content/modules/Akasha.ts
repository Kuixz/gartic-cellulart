...
const versionToInterpreterMap: Map<
  string,
  (match1: string, match2: string) => AkashicRecord
> = new Map([
  [
    "1.1.0",
    (match1, match2) => {
...
export class Akasha extends CellulartModule {
  ...
  private constructInwindow(): Inwindow {
    ...
    const [drawBtn, saveBtn, uploadBtn, deleteBtn] =
  ...
  private createDownloadButton(
    ...
    newButton.innerHTML = createIconHTML("cellulart-akasha-download", {
      type: "div",
    });
  ...
  private addRecord(newRecord: AkashicRecord) {
    newRecord.element.addEventListener("click", () => {
      this.focusRecord(newRecord);
    });

    this.records.add(newRecord);
    this.inwindow.body
      .querySelector(".akasha-album")!
      .appendChild(newRecord.element);
  }
