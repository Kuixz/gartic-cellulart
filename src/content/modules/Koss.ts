...
export class Koss extends CellulartModule {
  // [K1]
  name = "Koss";
  ...
  protected onphasechange(event: PhaseChangeEvent): void {
    ...
    if (newPhase == "memory") {
      this.discardCanvas();
      setTimeout(() => {
        this.kossCanvas = document.querySelector(
          ".core canvas",
  ...
  private tryPlace() {
    ...
    if (this.isOff()) {
      ...
      if (!drawingContainer) {
        Console.warn(
          "Cannot underlay canvas: cannot find active drawing canvas",
          "Koss",
        );
        return;
      }
