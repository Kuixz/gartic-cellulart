...
export class Geom extends CellulartModule {
  ...
  private initGeomWIW(): Inwindow {
    ...
    const startGeometrize = (files: FileList | null) => {
      // [G1]
      if (!files) {
        return;
      }
      ...
      buffer.addEventListener("setstrokegeneration", (e: Event) => {
  ...
  async geometrize(img: HTMLImageElement) {
    const resizedDimensions = viewFit(
      758,
      424,
      img.naturalWidth,
      img.naturalHeight
    ...
    this.queryGW("set", { width: 758, height: 424, data: imgdata.data }).then(
      (response) => {
        if (response.status != 200) {
          Console.log("Could not recognise imagedata", "Geom");
          return;
        }
        Console.log(
    ...
    const step = async () => {
      ...
      if (this.shapeGenerationPaused) {
        // || this.counters.created >= this.config.max || this.counters.created - this.counters.sent >= this.config.distance) {
      ...
      if (this.strokeBuffer == null || this.strokeBuffer !== buffer) {
        return;
      }
