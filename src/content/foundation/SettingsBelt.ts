...
class SettingsBelt {
  ...
  get current() {
    return this.items[this.index] || this.extension;
  }
  isSetTo(internalName: string): boolean {
    return this.current.internalName == internalName;
  }
  next() {
  ...
  jump(settingName: string) {
    while (this.current.internalName != settingName) {
      this.next();
    }
  }
}
...
function SettingFrom(settingName: string, overrideAsset?: string): Setting {
  return new Setting(
    settingName,
    overrideAsset ? getMenuIcon(`${overrideAsset}.png`) : undefined,
  );
}

function SettingsBeltFrom(
  moduleName: string,
  settingNames: string[],
  defaultIndex: number,
  extension?: string,
  ...
  return new SettingsBelt(
    settingNames.map((s) =>
      SettingFrom(
        s,
        s in DefaultSettingsValues ? undefined : `${moduleName}-${s}`,
      ),
    ),
    defaultIndex,
...
/** State order: Off, On */
function WhiteSettingsBelt(
  moduleName: string,
  enabledByDefault: boolean = false,
  extension?: string,
): SettingsBelt {
  return SettingsBeltFrom(
    moduleName,
    [DefaultSettings.off, DefaultSettings.on],
