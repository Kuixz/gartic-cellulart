class Console {
  ...
  enabledLoggingFor: Set<string | undefined> = new Set([
    "Console",
    undefined,
    "Socket",
    "Observer",
  ...
  set(modName: string, enabled: boolean) {
    enabled
      ? this.enabledLoggingFor.add(modName)
      : this.enabledLoggingFor.delete(modName);
    this.log(
      (enabled ? "Enabled " : "Disabled ") + "logging for " + modName,
      "Console",
    );
  }
  log(message: string, modName?: string) {
    if (!this.shouldLog || !this.enabledLoggingFor.has(modName)) {
      return;
    }
