...
export function constructElement(
  ...
  referenceRecord?: Record<string, HTMLElement>,
): HTMLElement {
  ...
  if (def.style) {
    elem.style.cssText = def.style;
  }
  if (def.textContent) {
    elem.textContent = def.textContent;
  }
  if (def.reference && referenceRecord) {
    referenceRecord[def.reference] = elem;
  }
  if (def.properties) {
    for (const [k, v] of Object.entries(def.properties)) {
      if (k in elem) {
        (elem as Record<string, any>)[k] = v;
      }
    }
  }
  ...
  if (def.children) {
    for (const childDef of def.children) {
      const child =
        childDef instanceof HTMLElement
          ? childDef
          : constructElement(childDef, referenceRecord);
