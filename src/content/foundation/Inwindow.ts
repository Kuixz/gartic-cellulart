...
setAttributes(Default.InwindowNode, {
  style: "visibility: hidden",
  class: "window-in-window",
});
...
class InwindowElement {
  ...
  constructor(
    ...
    const e =
      element == "default"
        ? (Default.InwindowNode.cloneNode(true) as HTMLElement)
        : element;
    this.element = e;
  ...
  if (options) {
    if (options.width && options.height && options.ratio) {
      if (options.width != options.ratio * options.height) {
        Console.warn(
          "Inconsistent dimensions supplied to Inwindow constructor",
        );
      }
...
function initRemoveElement(
  ...
  if (closeable) {
    inwindow.close.addEventListener("click", () => {
