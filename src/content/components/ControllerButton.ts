  ...
  switch (setting.internalName) {
    ...
    default:
      ...
      if (setting.overrideAssetPath) {
        ...
        Console.log(
          `Setting ${setting.internalName} is nonstandard but no overrideAssetPath was given"`,
          "Controller",
        );
        button.classList.remove("asset-override");
      }
...
function createButton(
  ...
  item.addEventListener("click", function () {
    ...
    if (newSetting) {
      updateButtonTexture(item, newSetting);
    }
  });
...
fetch(getResource("assets/menu-svgs.html"))
  .then((r) => r.text())
  .then((html) => {
