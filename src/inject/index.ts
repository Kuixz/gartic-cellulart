...
abstract class Interceptor {
  ...
  constructor() {
    ...
    window.addEventListener("message", (event: CrossCommand) => {
      if (
        event.source !== window ||
        event.data.direction !== `to${this.name}`
      ) {
        return;
      }
  ...
  protected interceptIncoming(str: string): Maybe<string> {
    return undefined;
  }
  protected interceptOutgoing(str: string): Maybe<string> {
    return undefined;
  }
...
class SocketInterceptor extends Interceptor {
  ...
  protected interceptOutgoing(data: string): Maybe<string> {
    ...
    if (data.slice(0, 8) == "42[2,18,") {
      this.readIncoming(data);
      return data;
    }
...
class XHRInterceptor extends Interceptor {
  ...
  interceptIncoming(data: string): Maybe<string> {
    ...
    if (indexFirstBracket < 0 || data.indexOf("{") < indexFirstBracket) {
      return data;
    }
    const json = JSON.parse(data.slice(indexFirstBracket)) as [
      number,
      object,
      ...any,
    ];
