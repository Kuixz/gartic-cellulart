import { Bitmap, ImageRunner as CoreRunner, ImageRunnerOptions, ShapeTypes } from 'geometrizejs'
...
chrome.runtime.onMessage.addListener(
    function(request, sender, sendResponse) {
        if (request.function == "set") {
            ...
            if (!runner) { sendResponse("Image is missing or expired"); return }
            const step = runner.step(DEFAULT_RUNNER_OPTIONS)
            const shape = step[0]
            const response = { type: shape.shape.getType(), raw: shape.shape.getRawShapeData(), svg: shape.shape.getSvgShapeData(), color: shape.color }
